...
void isr_timer(uint32_t irq, struct context *ctx)
{
    ...
    if(g_task_running != NULL) {
        ...
        if(g_task_running->tid == 0) {
            g_resched = 1;
        } else {//这里写属性值改变判断是不是一秒才行要考虑没有运行和task0
            g_task_running->estcpu = fixedpt_add(g_task_running->estcpu,FIXEDPT_ONE);
            if(g_timer_ticks%HZ==0){//每秒钟发生 == estcpu和gloadavg更新 PS:如何计算nready应该针对此刻循环
            //g_load_avg = g_load_avg*(59/60)+nready/60;
            //estcpu = (2*g_load_avg)/(2*g_load_avg+1)*estcpu+nice;
            int nready = 0;
            struct tcb *select = g_task_head;fixedpt ratio;
            ratio = fixedpt_mul(FIXEDPT_TWO, g_load_avg);
            ratio = fixedpt_div(ratio, fixedpt_add(ratio, FIXEDPT_ONE));
            while(select!=NULL) {//直接从头循环到尾计数
