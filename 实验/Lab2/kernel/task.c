...
void schedule()
{//进来就先计算priority
	struct tcb* t = g_task_head;
	int priority_MIN = t->priority;
	while(t!=NULL){
		t->priority = PRI_USER_MAX-(fixedpt_toint(t->estcpu)/4)-(t->nice*2);
		if(t->priority<priority_MIN) priority_MIN=t->priority;
		t = t->next;
	}
	//进行调度考虑：值越大优先级高，task0不管状态只管有没有其他线程ready
	

    struct tcb *select = g_task_head;
    t = task0;
    //int priority_MAX = priority_MIN;//原本想引用预定义primin不确定是不是最小还是老实求最小优先级；===其实不用管t初始化优先级也可以如下
    while(select!=NULL){
        if((select->state==TASK_STATE_READY)&&(select!=task0)){//记得每次有and要括号
        	if(t->priority < select->priority||(t->tid==0)){//这里的另一个特例通过条件是否等于0非常关键； 这里select已经是ready只要t是task0就进入；
        		//priority_MAX = select->priority;
        		t = select;
        	}

        }
        select=select->next;
    }//第一次所有的while写的等于==逻辑问题
    if(t == g_task_running) {
        if(t->state == TASK_STATE_READY)
            return;
        t = task0;
    }
    if(t->signature != TASK_SIGNATURE)
        printk("warning: kernel stack of task #%d overflow!!!", t->tid);//我的无限重启bug来源于此 刚开始t用错select此时的select总是null 应该是这个->操作引发的(no 是printk?)
    // 轮转调度的算法如下do {
    //     select = select->next;
    //     if(select == NULL)
    //         select = g_task_head;
                            ...
                            void (*func)(void *pv), void *pv)
{
    ...
    new->kstack = (uint32_t)(p+PAGE_SIZE);//后面初始化nice
    new->nice = 0;
    new->tid = tid++;
    new->estcpu = FIXEDPT_ONE;
...
int sys_setpriority(int tid,int prio){
	if((prio<0)&&(prio>=2*NZERO)) return -1;
	if(tid==0){
    	...
    	tsk=get_task(tid);
    	restore_flags(flags);
    	if(tsk!=NULL){tsk->nice = prio-NZERO;return 0;}
    	else return -1;
	}
}
