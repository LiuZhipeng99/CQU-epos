...
void syscall(struct context *ctx)
{
    ...
    case SYSCALL_getpriority:
        ctx->eax=sys_getpriority(*((int *)(ctx->esp+4)));//感觉返回0 -1的表状态没必要eax保存
        break;
    case SYSCALL_setpriority://这两个函数所需参数如何传递 参见taskwiat
        ctx->eax=sys_setpriority(*((int *)(ctx->esp+4)),*((int *)(ctx->esp+8)));
        break;
