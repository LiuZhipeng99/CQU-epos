 ...
 */
//这里总结报错：1.变量的声明不应该出现在label之后，labe指的是case 2.另一个方法此块代码不能再声明变量，如果你声明变量就会报错，除非用括号括起来
void syscall(struct context *ctx)
{
    //time_t *loc=0;//提前声明解决了报错a label can only be part of a statement and a declaration is not a statement
    //printk("task #%d syscalling #%d.\r\n", sys_task_getid(), ctx->eax);
    switch(ctx->eax) {
    case SYSCALL_time:{
        time_t *loc=*(time_t **)(ctx->esp+4);//类型和*号不能连着??

        ctx->eax = sys_time();
        if(loc!=NULL) *loc = ctx->eax;}
        break;

    case SYSCALL_sem_create:
        ctx->eax = (uint32_t)sys_sem_create(*((int*)(ctx->esp+4)));//返回值是空指针，传给ctx的eax会报警告
        break;
    case SYSCALL_sem_destroy:
        ctx->eax = sys_sem_destroy(*((void**)(ctx->esp+4)));
        break;
    case SYSCALL_sem_wait:
        ctx->eax = sys_sem_wait(*((void**)(ctx->esp+4)));
        break;
    case SYSCALL_sem_signal:
        ctx->eax = sys_sem_signal(*((void**)(ctx->esp+4)));
        break;
